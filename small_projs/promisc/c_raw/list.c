...
void
list_add(struct list *root, struct list_item *item)
{
...
void
list_add_to_end(struct list *root, struct list_item *item)
{
	assert(item != NULL && root != NULL);
...
int
list_pass(struct list *root, list_pass_cb func)
{
	struct list_item *tmp;
	int i = 0;
...
list_sort(struct list *root, compare_func_t func)
{
	...
	list_len = list_pass(root, NULL);
	
	if (list_len < 2)		//no data to sort
		return 0;
	
	stack_len = (int)lg(list_len) + 1;
