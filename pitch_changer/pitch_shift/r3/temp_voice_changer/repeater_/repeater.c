...
main(int argc, char *argv[])
{
	...
	res = snd_pcm_delay(pcm_c, &delay);
	if (res != 0) {
		fprintf(stderr, "unable to set delay(: %s\n", snd_strerror(res));
		exit(1);
	}
	...
	snd_pcm_status(pcm_c, status);

	printf("delay %d\n", delay);
	*/
	////set params^__^
	res = snd_pcm_set_params(pcm_p, SND_PCM_FORMAT_FLOAT_LE,
				     SND_PCM_ACCESS_RW_INTERLEAVED, 1, 44100, 0, 50);
	...
	res = snd_pcm_set_params(pcm_c, SND_PCM_FORMAT_FLOAT_LE, SND_PCM_ACCESS_RW_INTERLEAVED, 1, 44100, 0, 50);
	if (res != 0) {
		fprintf(stderr, "unable to set delay(: %s\n", snd_strerror(res));
		exit(1);
	}
