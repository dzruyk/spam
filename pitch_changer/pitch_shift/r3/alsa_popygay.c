...
main(int argc, char *argv[])
{
	...
	res = snd_pcm_open(&pcm_c, name, SND_PCM_STREAM_CAPTURE, SND_PCM_ASYNC);
	if (res != 0) {
		printf("open for capture err\n");
		exit(1);
	}
	
	res = snd_pcm_open(&pcm_p, name, SND_PCM_STREAM_PLAYBACK,  SND_PCM_NONBLOCK);
	if (res != 0) {
	...
	while (loop-- != 0) {
		...
		smbPitchShift(shift, frames, 512, 4, 44100, ibuff, obuff);
