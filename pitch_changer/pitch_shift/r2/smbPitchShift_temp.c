...
* data in-place). N defines the FFT frame size used for the
...
void smbFft(float *fftBuffer, long N, long sign);
...
void
smbPitchShift(float pitchShift, long numSampsToProcess, long N, long osamp, float sampleRate, float *indata, float *outdata)
/*
...
*/
{
	...
	static long gRover, gInit = false;
	double magn, phase, tmp, window, real, imag;
	double freqPerBin, expct;
	long i,k, qpd, index, inFifoLatency, stepSize, N2;

	/* set up some handy variables */
	N2 = N / 2;
	stepSize = N / osamp;
	freqPerBin = sampleRate / (double)N;
	expct = 2. * M_PI * (double)stepSize / (double)N;
	inFifoLatency = N - stepSize;

	gRover = inFifoLatency;

	/* initialize our static arrays */				//ШОЗАХУЙНЯ???? кода убираешь ифы всё НАХУЙРАЗВАЛИВАЕТСЯ
	if (gInit == false) {
	...
	for (i = 0; i < numSampsToProcess; i++){
		...
		if (gRover >= N) {
			...
			/* Hann Window. */
			for (k = 0; k < N; k++) {
				window = -0.5 * cos(2.0 * M_PI * (double)k / (double)N) + 0.5;
			...
			memset(gSynMagn, 0, N * sizeof(float));
			memset(gSynFreq, 0, N * sizeof(float));
			for (k = 0; k <= N2; k++) { 
				index = k * pitchShift;
				if (index <= N2) { 
			...
			for(k=0; k < N; k++) {
				window = -0.5 * cos(2.0 * M_PI * (double)k / (double)N) + 0.5;
				gOutputAccum[k] += 2.0 * window * gFFTworksp[2 * k] / (N2 * osamp);
			}
			for (k = 0; k < stepSize; k++)
				gOutFIFO[k] = gOutputAccum[k];

			/* shift accumulator */
			memmove(gOutputAccum, gOutputAccum+stepSize, N * sizeof(float));
...
/* 
	...
	Fills fftBuffer[0...2*N-1] with the Fourier transform of the
	time domain data in fftBuffer[0...2*N-1]. The FFT array takes
	and returns the cosine and sine parts in an interleaved manner, ie.
	fftBuffer[0] = cosPart[0], fftBuffer[1] = sinPart[0], asf. N
	must be a power of 2. It expects a complex input signal (see footnote 2),
	ie. when working with 'common' audio signals our input signal has to be
	passed as {in[0],0.,in[1],0.,in[2],0.,...} asf. In that case, the transform
	of the frequencies of interest is in fftBuffer[0...N].
*/
{
	...
	for (k = 0, le = 2; k < (long)(log(N) / log(2.0) + 0.5); k++) {
		le <<= 1;
		le2 = le >> 1;
		ur = 1.0;
		ui = 0.0;
