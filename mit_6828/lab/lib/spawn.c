...
spawn(const char *prog, const char **argv)
{
	...
	//cprintf("spawn of %s begining (from %d)\n", prog, sys_getenvid());
	...
	for (i = 0; i < UTOP / PGSIZE; i++) {
		...
		//cprintf("prog %s addr = %p child %d me %d\n", prog,  i * PGSIZE, child, sys_getenvid());
		r = sys_page_map(0, (void *) (i * PGSIZE), child,
		    (void *) (i * PGSIZE), perm);
		if (r != 0)
			panic("can't map page %e srcenv = %d dstenv = %d", r, 0, child);
	}
