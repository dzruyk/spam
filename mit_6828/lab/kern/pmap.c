...
page_init(void)
{
	...
	allocated = (int)boot_alloc(0) - KERNBASE; 
	for (i; i < allocated / PGSIZE; i++)
		pages[i].pp_ref = 1;
...
// a pointer to the page table entry (PTE) for linear addr`ess 'va'.
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	if (size + base > MMIOLIM)
		panic("mmio_map_region can't alloc mem");
	boot_map_region(kern_pgdir, base, sz, pa,
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	//is va points to kernel memory?
	if ((uint32_t) va > ULIM) {
		user_mem_check_addr = (uintptr_t)va;
		return -E_FAULT;
	} else if ((uint32_t) va > (uint32_t)va + len) {
	...
	//check first va 
	for (addr = (uint32_t)va; 
	    addr < end;
	    addr = ROUNDDOWN(addr + PGSIZE, PGSIZE)) {
		struct PageInfo *pi;
		
		pi = page_lookup(pgdir, (void *)addr, &pp);
		if (pi == NULL ||
		    ((uint32_t)pp & (perm | PTE_P)) == (perm | PTE_P)) {
